...
@torch.no_grad()
def evaluate(val_loader: DataLoader[Sample], model: SKTR, global_step: int = 0) -> EvaluationMetrics:
    total_loss = 0.0
    count = 0
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Evaluating"):
            ...
            eval_store.upsert(
                ...
                paths=[f"photo_{global_step}_{photo_idx}" for photo_idx in range(batch_size)],
                categories=batch["categories"],
            )
            mask = _create_class_mask(batch["categories"]).to(DEVICE)
            loss = sup_con_loss(
                pairviews(torch.cat([photo_embeddings, sketch_embeddings], dim=0).to(DEVICE)),
                mask=mask
            )
        ...
        for batch in tqdm(val_loader, desc="Evaluating retrieval"):
            ...
            map_at_10 += eval_store.mean_average_precision_at_k(
...
def train():
    ...
    lr_scheduler = build_warmup_cosine_scheduler(optimizer, total_steps=total_steps, warmup_steps=warmup_steps, min_lr_ratio=0.1)
    run_name = time.strftime("%Y%m%d-%H%M%S")
    writer = SummaryWriter(log_dir=f"runs/{run_name}")
    scaler = torch.amp.GradScaler("cuda", enabled=True)
    for epoch in range(epochs := CFG.training.epochs):
        progress_bar = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{epochs}", miniters=50)
        for batch_index, batch in enumerate(progress_bar):
            ...
            if (global_step_number + 1) % 50 == 0:
                ...
                writer.add_scalar("train/lr", lr_scheduler.get_last_lr()[0], global_step_number)
            ...
            if (global_step_number + 1) % CFG.validation.eval_every_steps == 0 or is_last_step:
                eval_metrics = evaluate(val_loader, model, global_step=global_step_number)
                writer.add_scalar("eval/loss", eval_metrics.loss, global_step_number)
                writer.add_scalar("eval/mAP@10", eval_metrics.mean_average_precision_at_10, global_step_number)
                tqdm.write(
                    f"Eval at epoch {epoch + 1}, step {global_step_number + 1}: "
                    f"loss={eval_metrics.loss:.4f}, "
                    f"mAP@10={eval_metrics.mean_average_precision_at_10:.4f}, "
                )
        model_save_path = Path(CFG.training.model_save_path) / f"sktr_model_epoch_{epoch + 1}.pth"
