...
export const imageQueryKeys = {
  ...
  list: (query: ImageSearchQuery) => [...imageQueryKeys.lists(), query] as const,
} as const;
...
export function useFsEvents({onCreate, onDelete, onModify, onMove}: Partial<UseFsEventsOptions>) {
  useEffect(() => {
    sseFsEventsClient.acquire()
    const removers: (() => void)[] = []
    if (onCreate) {
      removers.push(sseFsEventsClient.addListener("FileCreatedEvent", onCreate))
    }
    if (onDelete) {
      removers.push(sseFsEventsClient.addListener("FileDeletedEvent", onDelete))
    }
    if (onModify) {
      removers.push(sseFsEventsClient.addListener("FileModifiedEvent", onModify))
    }
    if (onMove) {
