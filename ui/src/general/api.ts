...
export async function apiFetch<TData>({url, context, ...init}: ApiFetchOptions): Promise<TData> {
  let response: Response;
  try {
    response = await fetch(url, {...init})
  } catch (error) {
    throw new ApiError(error instanceof Error ? error.message : "Network Error", context)
  }
  let body: ApiResponse<TData>;
  try {
    body = await response.json()
  } catch (error) {
    throw new ApiError(error instanceof Error ? error.message : "Could not parse JSON Response", context, response)
  }
